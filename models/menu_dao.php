...
class menu_dao
{
    ...
    function getMenuByIDs(array $ids): array
    {
        return DB::select("SELECT * FROM `" . self::$table_name . "` WHERE `id` IN (" . implode(",", $ids) . ")");
    }

    function getMenuSettingAll()
    {
        $dataArr = DB::select("SELECT * FROM `" . self::$table_name . "`");
        return $dataArr;
    }
